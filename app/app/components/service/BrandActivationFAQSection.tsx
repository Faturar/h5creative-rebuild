...
import { brandActivationFAQ } from "@/app/constants/faq"
...
export default function BrandActivationFAQSection() {
  const [active, setActive] = useState<number | null>(
    brandActivationFAQ.defaultActiveIndex,
  )
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-start">
          ...
          <motion.div
            ...
            variants={containerVariants}
          >
            {brandActivationFAQ.faqs.map((faq) => {
                  ...
                  >
                    <motion.span
                      className={`text-lg md:text-xl font-semibold transition ${
                        isOpen ? "text-[#FF6B35]" : "text-gray-900"
                      }`}
                      whileHover={{ x: 5 }}
                    ...
                    <motion.div transition={{ duration: 0.3 }}>
                      <ChevronDown
                        className={`w-4 h-4 md:w-5 md:h-5 text-[#FF6B35] transition duration-300 ${
